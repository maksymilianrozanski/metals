  ...
  def batchSemanticdbTextDocuments(
    ...
    val futures = for {
      ...
      pc.batchSemanticdbTextDocuments(params.asJava, timeout)
        .asScala
        .flatMap { bytes =>
          val batched = s.TextDocuments.parseFrom(bytes).documents
          // The batch endpoint has a default implementation returning no
          // documents; presentation compilers that don't override it (e.g.
          // the external Scala 3 compiler) silently drop their whole group —
          // fetch the missing documents through the per-file endpoint.
          val produced = batched.map(_.uri).toSet
          val missing = params.filterNot(p => produced(p.uri.toString))
          if (missing.isEmpty) Future.successful(batched)
          else {
            val singles = missing.map { param =>
              pc.semanticdbTextDocument(param)
                .asScala
                .map { docBytes =>
                  val doc = s.TextDocument.parseFrom(docBytes)
                  if (doc.uri.isEmpty) None
                  else Some(doc.withUri(param.uri.toString))
                }
                .recover { case NonFatal(error) =>
                  scribe.debug(
                    s"semanticdb fallback failed for ${param.uri}: $error"
                  )
                  None
                }
            }
            Future.sequence(singles).map(extra => batched ++ extra.flatten)
          }
        }
    }
    Future
